...
public class CustomerResource {
    ...
    public Response getCustomerById(@PathParam(RESOURCE_PATH_ID_ELEMENT) int id) {
        ...
        if (securityContext.isCallerInRole(ADMIN_ROLE)) {
            cust = customerServiceBean.getCustomerById(id);
            response = Response.status( cust == null ? NOT_FOUND : OK).entity(cust).build();
        }
        else if (securityContext.isCallerInRole(USER_ROLE)) {
            WrappingCallerPrincipal wCallerPrincipal = (WrappingCallerPrincipal) securityContext.getCallerPrincipal();
    ...
    public Response addCustomer(CustomerPojo newCustomer) {
      Response response = null;
      CustomerPojo customer = customerServiceBean.persistCustomer(newCustomer);
      customerServiceBean.buildUserForNewCustomer(customer);
